...
import org.coolplugins.cool_HealthBar.model.HealthBarPercentage;
...
public abstract class AbstractEntityFormatter implements HealthBarLabel {
    ...
    public Component format(LivingEntity entity) {
        ...
        HealthBar bar = HealthBarConfigurationManager.getCustomHealthbar();
        double percentage = HealthBarPercentage.getPercentage(entity);
        Component mobHeader = getEntityHeader(entity);

        TextComponent.Builder builder = Component.text();

        builder.append(Objects.requireNonNull(bar).buildBarComponent(percentage));
